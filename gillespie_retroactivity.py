...
while t < t_max:
    ...
    a2 = k_on * C * T1                # Bind Target 1
    a3 = k_off * R1                   # Unbind Target 1
    a4 = k_on * C * T2                # Bind Target 2 (Decoy)
    a5 = k_off * R2                   # Unbind Target 2
...
# 6. Plotting the Stochastic Noise (OUTSIDE THE LOOP)
plt.figure(figsize=(8, 5))
plt.step(time_points, T1_points, where='post', label='Repressed Target 1 (Intended)', color='#00ff00', linewidth=2)
plt.step(time_points, T2_points, where='post', label='Repressed Target 2 (Decoy Load)', color='#ff0000', linewidth=2)
...
plt.title('Stochastic Gillespie: Circuit Failure Under Decoy Load')
plt.legend()
